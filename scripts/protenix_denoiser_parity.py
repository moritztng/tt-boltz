...
q_l=ttnn.add(c_la,lin(T(x_noisy[0]),'linear_no_bias_r.weight',E))
def wq(x):
...
z_t=T(pair_z.unsqueeze(0)); a_t=ttnn.reshape(a_tok,(1,NT,768)); s_t=s_single
...
coords=torch.Tensor(ttnn.to_torch(lin(qn,'linear_no_bias_out.weight',DE))).float().reshape(1,N,3)[:,:N]
def pcc(u,v):
