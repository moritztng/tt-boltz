...
def _post(mod,a,o):
    if 'out' not in cap: cap['out']=o.detach().clone() if torch.is_tensor(o) else tuple(x.detach().clone() for x in o)
aae=m.diffusion_module.atom_attention_decoder
...
pickle.dump(cap, open('/home/ttuser/protenix_atomdec_pre.pkl','wb'))
print('atomdec pre: in',len(cap['in']),'kwargs',list(cap['kwargs']),'out',type(cap['out']).__name__, tuple(cap['out'].shape) if torch.is_tensor(cap['out']) else len(cap['out']),flush=True)
