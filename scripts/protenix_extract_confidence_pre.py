...
def _post(mod,a,o):
    if 'out' not in cap: cap['out']=tuple(x.detach().clone() if torch.is_tensor(x) else x for x in o) if isinstance(o,(list,tuple)) else o.detach().clone()
aae=m.confidence_head
...
pickle.dump(cap, open('/home/ttuser/protenix_confidence_pre.pkl','wb'))
print('confidence pre: kwargs',list(cap['kwargs']),'out',len(cap['out']) if isinstance(cap['out'],tuple) else 'T',flush=True)
