...
cap={}
blocks=m.diffusion_module.diffusion_transformer.blocks
def mk(i):
    def h(mod,a,o):
...
pickle.dump({i:cap[i] for i in cap}, open('/home/ttuser/protenix_dit_blocks_gold.pkl','wb'))
print('captured DiT per-block outputs:', len(cap), 'block0 shape', tuple(cap[0].shape), flush=True)
