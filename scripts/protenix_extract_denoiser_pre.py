...
pickle.dump(cap, open('/home/ttuser/protenix_denoiser_pre.pkl','wb'))
print('denoiser pre: kwargs',list(cap['kwargs']),'out',type(cap['out']).__name__,tuple(cap['out'].shape) if torch.is_tensor(cap['out']) else len(cap['out']),flush=True)
